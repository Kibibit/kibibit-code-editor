	...
	apiRouter.route('/file/:file_id')
		...
		.put(function(req, res) {
			...
			try {
				...
				res.json({"errno": "330","code": "File already exists","path": fileFullPath});
		...
		.put(function(req, res) {
			...
			if (isHardSave === "true") {
				if (req.body.newContent|| 0 === req.body.newContent.length) {
					...
					res.json({ message: 'you have to include a body with newContent' });
				}
			} else {
				res.json({ message: 'For a hard save, 2nd param should be: True' });
			}
		...
		.put(function(req, res) {
			...
			fs.rename(directoryFullPath, newDirectoryFullPath, function (err) {
				if (err) {
					...
					console.time().tag('DIRECTORY RENAME').info('directory renamed from: ' + directoryFullPath
						+ '; to: ' + newDirectoryFullPath);
		...
		.delete(function(req, res) {
			...
			try {
				if (isHardDelete === "true") {
					...
					res.json({ message: 'Directory deleted (recursively) successfully' });
					console.time().tag('DIRECTORY DELETED').info('directory deleted (recursively) successfully: ' + directoryFullPath);
				} else {
					res.json({ message: 'For a hard delete, 2nd param should be: True' });
				}
			} catch(err) {
				res.json(err);
